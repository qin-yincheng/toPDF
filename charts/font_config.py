...
def setup_chinese_font() -> None:
    ...
    if not selected_font:
        ...
        for font in available_fonts:
            if any(keyword in font for keyword in ['CJK', 'Chinese', '黑体', '宋体', '微软']):
                selected_font = font
...
def test_chinese_font():
    ...
    for text, y in zip(test_texts, y_positions):
        ax.text(0.5, y, text, fontsize=14, ha='center', va='center',
...
if __name__ == '__main__':
    ...
    chinese_fonts = [f for f in sorted(available) if any(
        keyword in f for keyword in [
            'CJK', 'Chinese', 'PingFang', 'Heiti', 'Songti', 
            'SimHei', 'SimSun', 'Microsoft', 'YaHei', 'KaiTi',
            'WenQuanYi', 'Noto', 'Droid', 'Arial Unicode'
        ]
    )]
