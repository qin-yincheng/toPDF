...
def plot_asset_allocation_chart(
    ...
    ax.bar(x_positions, funds, width=bar_width, bottom=bottom1, label='基金', color=color_funds, edgecolor='white', linewidth=0.5)
    ax.bar(x_positions, reverse_repurchase, width=bar_width, bottom=bottom2, label='逆回购', 
           color=color_reverse_repurchase, edgecolor='white', linewidth=0.5)
    ax.bar(x_positions, cash, width=bar_width, bottom=bottom3, label='现金', 
           color=color_cash, edgecolor='white', linewidth=0.5)
    ax.bar(x_positions, other_assets, width=bar_width, bottom=bottom4, label='其他资产占比', 
           color=color_other, edgecolor='white', linewidth=0.5)
    
    # 设置Y轴（图片中Y轴范围是0-120%）
    ax.set_ylabel('占比(%)', color='black')
    ax.set_ylim(0, 120)
    ...
    # 设置标题（如果启用）
    if show_title:
        ax.set_title('大类持仓时序', fontsize=14, fontweight='bold', pad=20, loc='left')
...
def _generate_mock_asset_allocation_data() -> List[Dict[str, Any]]:
    ...
    for i, date in enumerate(dates):
        ...
        if date < datetime(2024, 9, 1):
            ...
            # 平滑过渡
            stocks = prev_stocks * 0.7 + target_stocks * 0.3 + np.random.uniform(-0.5, 0.5)
            cash = 100 - stocks
        elif date < datetime(2024, 9, 15):
            # 9月初：股票降到约80%
            target_stocks = 80.0 + np.random.uniform(-1, 1)
            stocks = prev_stocks * 0.8 + target_stocks * 0.2 + np.random.uniform(-0.3, 0.3)
            cash = 100 - stocks
        elif date < datetime(2024, 10, 1):
            # 9月中下旬：股票回升到85-90%
            target_stocks = 87.0 + np.random.uniform(-2, 2)
            stocks = prev_stocks * 0.85 + target_stocks * 0.15 + np.random.uniform(-0.3, 0.3)
            cash = 100 - stocks
        elif date < datetime(2024, 12, 15):
            # 10-11月到12月中旬：股票接近100%（95-100%），偶尔有少量现金
            target_stocks = 98.0 + np.random.uniform(-1, 1)
            stocks = prev_stocks * 0.9 + target_stocks * 0.1 + np.random.uniform(-0.2, 0.2)
            cash = 100 - stocks
            ...
            stocks = prev_stocks * 0.6 + target_stocks * 0.4 + np.random.uniform(-0.5, 0.5)
            ...
            reverse_repurchase = prev_reverse_repurchase * 0.5 + target_reverse_repurchase * 0.5 + np.random.uniform(-0.5, 0.5)
            cash = 5.0 + np.random.uniform(0, 3)
            ...
            reverse_repurchase = prev_reverse_repurchase * 0.8 + target_reverse_repurchase * 0.2 + np.random.uniform(-0.3, 0.3)
