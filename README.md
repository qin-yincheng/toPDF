...
- 🔄 **实时计算**：支持自定义统计区间和基准指数
- 📅 **年度报告**：支持按年份生成报告，自动处理跨年数据一致性
- 📊 **基准对比**：沪深300等基准指数动态归一化对比
- 🎨 **跨平台字体**：macOS/Windows/Linux自动适配中文显示
...
# 报告年份限制（可选）
REPORT_YEAR = None                   # None=全部数据，"2016"=仅2016年
# 设置后会：
# 1. 筛选指定年份的交易和持仓数据
# 2. 期初资产使用上一年12-31数据（确保跨年一致性）
# 3. 基准归一化到该年第一个交易日
# 4. 统计区间自动调整为该年度范围
```
...
**症状**：生成2016年报告时，期初资产(4005.18万)与2015年期末(3994.47万)不一致

**原因**：元旦(2016-01-01)不是交易日，图表使用首个交易日(2016-01-04)数据

**已修复**：v1.2.0版本已修复，图表使用日历期初，确保跨年一致性

**Q5: 沪深300基准线显示为平线**

**症状**：股票仓位时序和流动性资产时序中基准线全部为1.0

**原因**：基准数据未传递或归一化基准点错误

**已修复**：v1.2.0版本已修复，基准线正常显示市场波动

**Q6: `list index out of range` 或其他数据错误**
...
✅ **数据一致性修复**
- 修复跨年期初资产不一致问题
  - 问题：2016年期初资产(4005.18万)与2015年期末(3994.47万)不一致
  - 原因：元旦非交易日导致图表使用首个交易日(1-04)而非日历期初(1-01)
  - 方案：在 `charts/1_2.py` 中保存日历期初数据，确保期初=上年期末
  - 结果：✅ 跨年数据连续性保证，期初资产正确显示3994.47万元

✅ **基准数据修复**
- 修复沪深300基准线显示为平线的问题
  - 问题：股票仓位时序和流动性资产时序中基准线全部为1.0，无波动
  - 原因1：`build_asset_allocation_series`未传入基准净值数据
  - 原因2：基准归一化到所有数据第一天(2015-01-05)而非报告年份第一天
  - 方案：
    - 修改 `calc/report_bridge.py` 传递基准净值数据到资产配置序列
    - 修改 `generate_report.py` 基准归一化逻辑，使用报告年份第一个交易日
    - 修改 `charts/2_4.py` 和 `charts/2_5.py` 动态计算Y轴范围
  - 结果：✅ 基准线正常显示市场波动(2016年波动24.92%)

🎯 **改进要点**
- 期初资产定义：使用日历期初(X-01-01)而非交易期初(首个交易日)
- 基准归一化：按报告年份归一化，确保年度对比的准确性
- Y轴自适应：图表Y轴范围根据实际数据动态调整(添加10%边距)
